...
# else:
#     modelNames = ['Group-LMM', 'Group Lasso', 'LMM']
...
def visualize(cat):
    ...
    we=0.05
    if cat == 'n':
        valueList = [800,1000,1500,2000]
        name = 'n'
    elif cat == 'n2':
        valueList = [800, 1000, 1500, 2000]
        name = 'n'
    elif cat == 'p':
        valueList = [1000, 2000, 5000,8000]
        name = 'p'
    elif cat == 'd':
        valueList = [ 0.01,0.03 ,0.05,0.1]
        name = 'd'
    elif cat == 'g':
        valueList = [5, 10, 20, 50]
        name = 'G'
    elif cat == 'k':
        valueList = [ 5,10, 50, 100]
        name = 'K'
    elif cat == 's':
        valueList =  [0.0001, 0.0005, 0.001, 0.002]
        name = r'$\sigma_e^2$'
    elif cat=='we':
        ...
        name = r'$\sigma_r^2$'
    if cat == 'n2':
        fig = plt.figure(dpi=500, figsize=(20, 8))
    else:
    ...
    for i in range(len(valueList)):
        if cat == 'n2':
            axs[i] =fig.add_axes([0.06 + (i) * 0.22, 0.1, 0.19, 0.50])
        else:
        ...
        for j in range(modelNum):
            fpr, tpr, t = evaluation(L, B[j])
            s=roc_auc_score(L, B[j])
